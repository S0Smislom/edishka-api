...
func (r *RecipeRepository) getQuery(query *gorm.DB) *gorm.DB {
	return query.
		Joins("left join recipe_step on recipe.id=recipe_step.recipe_id").
		Joins("left join step_product on recipe_step.id = step_product.recipe_step_id").
		Joins("left join product on step_product.product_id = product.id")
}

func (r *RecipeRepository) getSelectQuery(query *gorm.DB) *gorm.DB {
	return query.Preload("Gallery").
		Select(`recipe.*,
			sum(coalesce(step_product.amount, 0) * coalesce(product.calories, 0)/100) as calories,
			sum(coalesce(step_product.amount, 0) * coalesce(product.fats, 0)/100) as fats,
			sum(coalesce(step_product.amount, 0) * coalesce(product.squirrels , 0)/100) as squirrels,
			sum(coalesce(step_product.amount, 0) * coalesce(product.carbohydrates , 0)/100) as carbohydrates`)
}
...
func (r *RecipeRepository) fetchProducts(recipes []*model.Recipe) {
	...
	res := r.db.Table("recipe").
		Select("product.*, sum(step_product.amount) as amount, recipe.id as recipe_id").
		Joins("inner join recipe_step on recipe_step.recipe_id=recipe.id").
		Joins("inner join step_product on recipe_step.id=step_product.recipe_step_id").
		Joins("inner join product on step_product.product_id=product.id").
